...
export class UserRepository implements IUserRepository {
    ...
    public async getFromEmail(email: string): Promise<UserData | undefined> {
        ...
        const user: any = await queryDatabase(
            connection,
            "SELECT id, email, password, name, firstName, lastName, authorizationLevel FROM user WHERE email = ?",
            email
        );
        connection.release();
    ...
    public async getFromId(id: number): Promise<UserData | undefined> {
        ...
        const user: any = await queryDatabase(
            connection,
            "SELECT id, email, password, name, firstName, lastName, authorizationLevel FROM user WHERE id = ?",
            id
        );
        connection.release();
        return user[0] as UserData | undefined;
    ...
    public async add(
        ...
        const connection: PoolConnection = await getConnection();
        const query: string =
            "INSERT INTO `user`(`email`, `password`, `name`, `firstName`, `lastName`) VALUES (?,?,?,?,?)";
